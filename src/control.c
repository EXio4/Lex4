...
void set_control(Tcontrol *c, int up, int down, int left, int right, int fire, int jump, int roll, int fly) {
	c->key_up     = up;
	c->key_down   = down;
	c->key_left   = left;
	c->key_right  = right;
...
void poll_control(Tcontrol *c) {
	...
	if (key[c->key_roll]) c->flags |= K_ROLL;
	if (key[c->key_fly]) c->flags |= K_FLY;
}
...
int check_control_key(Tcontrol *c, int key) {
	...
	if (key == c->key_roll) return TRUE;
	if (key == c->key_fly) return TRUE;
	return FALSE;
}
...
int is_roll(Tcontrol *c)     { return (c->flags & K_ROLL     ? TRUE : FALSE); }
int is_fly(Tcontrol *c)     { return (c->flags & K_FLY     ? TRUE : FALSE); }
