	...
	if (player.actor->status != AC_DEAD) {
		...
		if (is_fire(&ctrl) && player.actor->status == AC_FULL && !player.eat_pressed) {
			...
			if (player.ammo) {
				...
				if (b != NULL) {
					set_bullet(b, (int)player.actor->x + 7, (int)player.actor->y - 14, (player.actor->direction ? 4 : -4), 0, data[EGG2].dat, 0,1);
					player.ammo --;
...
int do_main_menu() {
	...
	if (status == GS_EDIT) {
		...
		if (map != NULL) {
			...
			editing = TRUE;
			set_edit_mode(EDIT_MODE_DRAW);
			set_edit_path_and_file("default.map");
